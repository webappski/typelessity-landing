...
import { SeoService, SITE_URL, type SeoLocale } from '../../core/seo/seo.service';
  ...
  template: `
    ...
    @if (i) {
      <article class="industry vc-wrap">
        <a class="industry__back" [routerLink]="'/' + t.lang() + '/industries'">← All industries</a>
        ...
        <header class="industry__hero">
          ...
          @if (i.hero.cta) {
            <div class="industry__cta">
              <a class="vc-btn vc-btn-primary vc-btn-lg" [routerLink]="'/' + t.lang() + '/pricing'">{{ i.hero.cta.primary }}</a>
              <a class="vc-btn vc-btn-ghost vc-btn-lg" [routerLink]="'/' + t.lang() + '/about'">{{ i.hero.cta.secondary }}</a>
            </div>
          }
        </header>
      ...
      <section class="vc-wrap" style="padding: 80px 0;">
        <h1>Industry not found</h1>
        <p><a [routerLink]="'/' + t.lang() + '/industries'">Back to industries</a></p>
      </section>
    }
  `,
})
export class IndustryDetailPageComponent {
  ...
  private buildServiceLd(slug: string, name: string, description: string): Record<string, unknown> {
    return {
      ...
      url: `${SITE_URL}/${this.t.lang()}/industries/${slug}`,
